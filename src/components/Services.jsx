...
const services = [
  {
    num: '01', tag: 'Remote Sensing', color: '#2563eb',
  ...
  {
    num: '02', tag: 'ESG & Carbon', color: '#16a34a', accentBg: '#dcfce7',
  ...
  {
    num: '03', tag: 'Crop Analytics', color: '#ea580c', accentBg: '#fff7ed',
  ...
  {
    num: '04', tag: 'Hydrology', color: '#2563eb',
        ...
        <div style={{
          ...
          {services.map((s, idx) => (
            <div key={s.num}
              ...
              style={{
              background: hovered === idx && s.accentBg ? s.accentBg : '#fff',
              padding: '44px 40px',
              display: 'flex', flexDirection: 'column',
              transition: 'box-shadow 0.3s, background 0.3s, border-color 0.2s',
              boxShadow: hovered === idx ? '0 8px 24px rgba(37,99,235,0.08)' : 'none',
              borderTop: `3px solid ${hovered === idx ? s.color : 'transparent'}`,
              position: 'relative',
            }}>
              <span style={{
                fontSize: '0.6rem', letterSpacing: '0.3em', textTransform: 'uppercase',
                color: s.color, fontWeight: 600, marginBottom: '8px',
              }}>
                {s.num} — {s.tag}
              </span>
              <h3 style={{
                ...
                {s.items.map(item => (
                  <li key={item} style={{
                    ...
                    <span style={{
                      ...
                      background: s.color, display: 'block',
                    }} />
                    {item}
                  </li>
                ))}
